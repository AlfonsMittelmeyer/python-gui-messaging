...
def mainloop():
	...
	#_Application.bind("<<SEND>>",proxy.do_work)
	#proxy.trigger()
	#proxy.set_trigger(lambda: _Application.event_generate("<<SEND>>", when="tail"))
	#proxy.extern_trigger = lambda func1 = root.after, func2 = root.event_generate: func1(0,lambda func = func2: func("<<SEND>>", when="tail"))
	#proxy.set_trigger(lambda: _Application.after(0,lambda: _Application.event_generate("<<SEND>>", when="tail")))
	#proxy.set_trigger(lambda func1 = root.after, func2 = root.event_generate: func1(0,lambda func = func2: func("<<SEND>>", when="tail")))
	#proxy.set_trigger(lambda: _Application.after(0,proxy.do_work))
...
def saveElement(filehandle):
	...
	thisClass = WidgetClass(this())
	filehandle.write(thisClass+"('"+name+"'")
...
def saveOneElement(filehandle,name):
	...
	for n,e in dictionaryCopy.items():
		if n == 'title':
			if e =="": dictionaryWidget.pop(n,None)
			 ...
			 if thisClass =="Listbox": dictionaryWidget.pop(n,None)
	...
	if this().hasWidgets():
		...
		filehandle.write("\ngoOut()\n")
		if this().Layout != PACKLAYOUT: addNl=True
		wasInContainer = True
	...
	if this().Layout & 6:
		...
		if this().Layout == GRIDLAYOUT:
			if wasInside: filehandle.write("grid(")
			else: filehandle.write(".grid(")
			CompareWidget.grid()
			layoutCompare = CompareWidget.grid_info()
		else: 
			if wasInside: filehandle.write("place(")
...
# wie wir es beim Einzelelement ha<tten, doch warum?
#
# ===================================================
