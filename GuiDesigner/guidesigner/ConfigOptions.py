def main(parent):
	...
	def show_config(msg,onflag = enable_flag, cont = container(),thisframe=widget("Frame"),color_action = do_color_action,text_color = do_text_color,color_button = create_color_button,e_event=entry_event,lbox_select=listbox_selection,wcanvas = widget('Canvas'),no_refresh=undo_refresh):
				...
				for entry in conflist:
					...
					if entry[0] == "text": do_event("<Return>",text_color,None,True)
						...
						if entry[0] == 'geometry':
							...
							msg.do_action('refresh',geometry_refresh,thisframe,True)
							inform(msg,'refresh')
